...
int main(){
    ...
    int window = getInfoFromRedis(c); int windowStart = 0;  // prendo la finestra da redis
    cout << window << endl;
    vector<string> streams = getStreamFromRedis(c); // prendo i nomi di tutte le stream
    ...
    for (int i = 0; i < streams.size(); i++){
        double avarange = getAvarange(streams[i], window, c);
