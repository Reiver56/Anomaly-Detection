...
int main(){
    ...
    vector<string> streams = getStreamFromRedis(); 
    ...
    try {
        ...
        for (int i = 0; i < streams.size()-1; i=i+2){
            ...
            W.exec_params(sql, i, streams[i], streams[i+1],ranges[i][0],ranges[i][1], ranges[i+1][0], ranges[i+1][1], getCovariance(streams[i], streams[i+1], averanges[i], averanges[i+1]));
            W.commit();
